...
-  En caso de sistema WINDOWSNT
    ...
    - Ejecutar en la shell de WSL el script. En algunos casos puede que aparezca un error del estilo ```/bin/bash^M: bad interpreter: No such file or directory```. Se corrige facilmente ejecutando en la carpeta del script ```sed -i -e 's/\r$//' kong_steps.sh```
